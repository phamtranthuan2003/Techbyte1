...
<div class="w-full max-w-lg p-8 bg-white shadow-xl rounded-2xl">
    <h1 class="text-2xl font-bold text-center text-gray-700 mb-6">Thêm mã giảm giá</h1>

    <form action="{{ route('admins.promotions.store') }}" method="post" class="space-y-4" enctype="multipart/form-data">
        @csrf
        <div>
            <label for="code" class="block text-sm font-semibold text-gray-700">Tên giảm giá</label>
        ...
        <div>
            ...
            <input type="number" id="discount" name="discount" required min="1"
                class="w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-300 focus:outline-none">
        </div>
