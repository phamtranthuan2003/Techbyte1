...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
    ...
    <div class="w-full max-w-lg p-8 bg-white shadow-xl rounded-2xl">
        ...
        <form action="{{ route('admins.colors.update', ['id' => $color->id]) }}" method="post" class="space-y-4">
            @csrf

            <!-- Tên danh mục -->
            <div>
                <label for="name" class="block text-sm font-semibold text-gray-700">Tên danh mục</label>
                <input type="text" id="name" name="name" placeholder="Nhập tên danh mục" required value="{{$color->name}}"
            ...
            <div>
                ...
                <input type="number" id="price" name="price" required min="1" value="{{ $color->price }}"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-300 focus:outline-none">
            </div>
