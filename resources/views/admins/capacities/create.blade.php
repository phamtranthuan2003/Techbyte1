...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
    ...
    <div class="w-full max-w-lg p-8 bg-white shadow-xl rounded-2xl">
        ...
        <form action="{{ route('admins.capacities.store') }}" method="POST" class="space-y-4">
            @csrf

            <!-- Tên danh mục -->
            <div>
                <label for="name" class="block text-sm font-semibold text-gray-700">Nhập dung lượng</label>
                <input type="text" id="name" name="name" placeholder="Nhập dung luong" required
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-300 focus:outline-none">
            </div>

            <!-- Mô tả danh mục -->
            <div>
            <label for="name" class="block text-sm font-semibold text-gray-700">Giá
